# Contributing to oxidized-mc-types
...
`oxidized-mc-types` is a foundational library crate used across the Oxidized MC
ecosystem. Key principles:

- **API stability matters** — public types are consumed by many downstream
...
1. Create a branch: `git checkout -b feat/block-pos-iterator`
...
- Reference the Java behaviour when writing expected values:
  ```rust
  // BlockPos packing matches net.minecraft.core.BlockPos in the reference
  assert_eq!(BlockPos::new(1, 2, 3).packed(), 0x0000_0003_0000_8001);
  ```
